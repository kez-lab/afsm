...
- Should Afsm add an optional v3 topology companion with `from<FromState> { on<Event>().to<ToState>() }` to support state diagram generation without static code analysis?
