...
- The remaining graph tooling release concerns are KSP processor compile tests, Gradle plugin functional tests, and multi-variant/multi-module graph generation policy.
...
- The first graph Gradle plugin slice is implemented. The remaining graph-tooling concerns are KSP processor compile-testing hardening, multi-variant/multi-module aggregation, and eventual graph API/module-boundary decisions before broad external adoption.
